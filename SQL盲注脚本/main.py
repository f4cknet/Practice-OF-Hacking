#-*-coding:utf8-*-
# tip: user_agent(random) | time.sleep(random.uniform())
#author:unclechow
#from hangzhou.china
...
if __name__ == '__main__':
    if len(sys.argv)>2 and sys.argv[2] == 'db_name':
        b = []
        ...
        print '\n'.join(b)+'\n'
        print "---------------------"      
    elif len(sys.argv)>2 and sys.argv[2] == 'db_version':
        l = []
        print "\n-------------开始猜解数据库版本-------------\n"
        for i in range(1,24):
            ...
            l.append(f)
        print ''.join(l)
    elif len(sys.argv)>2 and sys.argv[2] == 'table_name':
        b = []
        ...
        print '''usage: python main.py url <db_name | db_version | table_name num(获取第几个表名) | >'''  